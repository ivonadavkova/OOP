...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        DrawTextEx(font, "Text editor started", {10.0f, 10.0f}, 24, 0.7f, {0, 220, 200, 255});
        
        DrawLineEx({10.0f, 38.0f}, {static_cast<float>(GetScreenWidth() - 10), 38.0f}, 3.0f, {110, 80, 240, 255});

        string line = "";
        float y = 55.0f;
        for (int i = 0; i < letter_count; i++) {
            if (text[i] == '\n') {
                DrawTextEx(font, line.c_str(), {10.0f, y}, 18, 0.7f, {220, 225, 235, 255});
                y += line_height;
                line.clear();
            } else line += text[i];
        }
        ...
        if (static_cast<int>(GetTime() * 1.6) % 2) {
            ...
            DrawLineEx({10.0f + size.x, 52.0f + (row_of_cursor * line_height)}, {10.0f + size.x, 75.0f + (row_of_cursor * line_height)}, 3.5f, {255, 45, 140, 255});
        }
        EndDrawing();
    }
